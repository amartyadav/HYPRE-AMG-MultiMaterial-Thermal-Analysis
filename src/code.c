...
#define HYPRE_EXVIS 1  /* Add this manually */
...
int main (int argc, char *argv[])
{
   ...
   if (n * n < num_procs) { n = sqrt(num_procs) + 1; printf("n after adjustment: %d\n", n); }
   printf("n after adjustment: %d\n", n);
   ...
   */
   {
      ...
      */
      ...
      double k_alum = 200.0; // high conductivity
      double k_carb = 2.0; // low conductivity (insulator)
      ...
      for (i = ilower; i <= iupper; i++)
            {
               ...
               double k_self = (col < n/2) ? k_alum : k_carb;
               double diag = 0.0;
               double k_neighbor, weight;

               /* We calculate weights for all 4 directions.
                  If a neighbor is missing (at the boundary), we still add its
                  weight to the DIAGONAL to enforce Dirichlet conditions. */

               // 1. LEFT
               k_neighbor = (col > 0) ? ((col - 1) < n/2 ? k_alum : k_carb) : k_self;
               weight = (2.0 * k_self * k_neighbor) / (k_self + k_neighbor);
               diag += weight;
               if (col > 0) {
                  cols[nnz] = i - 1;
                  values[nnz] = -weight;
               ...
               // 2. RIGHT
               k_neighbor = (col < n - 1) ? ((col + 1) < n/2 ? k_alum : k_carb) : k_self;
               weight = (2.0 * k_self * k_neighbor) / (k_self + k_neighbor);
               diag += weight;
               if (col < n - 1) {
                  cols[nnz] = i + 1;
               ...
               // 3. TOP
               k_neighbor = k_self; // Vertical neighbor in same material zone
               weight = (2.0 * k_self * k_neighbor) / (k_self + k_neighbor);
               diag += weight;
               if (row > 0) {
                  cols[nnz] = i - n;
               ...
               // Now the diagonal is the sum of all 4 potential connections
               cols[nnz] = i;
               values[nnz] = diag;
               nnz++;
